...
import {ContractInfo, ContractPolicyInfo, IEventHandler, IPolicyService, IContractService} from '../../interface';
...
export class InitialContractEventHandler implements IEventHandler {
     ...
     * TODO: 后续服务需要提供定时JOB,用于扫描状态为Uninitialized的合约.然后对其初始化,防止部分合约初始化失败.
     */
    async handle(contractInfos: ContractInfo[]) {
        const contractPolicyMap: Map<string, ContractPolicyInfo> = await this.policyService.findByIds(contractInfos.map(x => x.policyId))
    ...
    async _initialContract(contract: { contractInfo: ContractInfo, policyInfo: ContractPolicyInfo }) {
